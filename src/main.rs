...
use esp_idf_svc::{
    ...
    http::{
        ...
        server::{Configuration as ServerConfig, EspHttpServer},
    },
    nvs::EspDefaultNvs,
    ping::EspPing,
};
...
fn main() -> Result<()> {
    ...
    let _wifi = match wifi_conf::wifi(
    ...
    */
    /*
        unsafe {
            log::info!("PSRAM size: {}", esp_psram_get_size());
        }
    */
    ...
    /*
        ...
        match camera {
            Ok(camera) => init_camera = camera,
            Err(e) => error!("Camera failed init with: {}", e),
        }
    */
    ...
    // Shared JPEG buffer (updated periodically)
    ...
    // --- Background camera capture thread ---
    {
        ...
        thread::spawn(move || {
            loop {
                if let Some(fb) = camera.lock().unwrap().get_framebuffer() {
                    *frame.lock().unwrap() = fb.data().to_vec();
                }
                thread::sleep(std::time::Duration::from_millis(10)); // ~10 FPS
            }
        });
    }
    ...
    // http://<sta ip>/ handler
    server.fn_handler("/h3alth", Method::Get, |request| {
        let html = index_html();
...

    {
        ...
        server.fn_handler("/camera.jpg", Method::Get, move |request| {
            // Lock camera for one frame
            let cam = camera.lock().unwrap();

            cam.get_framebuffer();
            // take two frames to get a fresh one
            let framebuffer = cam.get_framebuffer();
    ...
    // --- MJPEG streaming endpoint ---
    {
        ...
        server.fn_handler("/stream", Method::Get, move |req| {
            ...
            loop {
                ...
                resp.write_all(frame_header.as_bytes())?;
                resp.write_all(&jpeg)?;
                resp.write_all(b"\r\n")?;
    ...
    /*
        {
            ...
            server.fn_handler("/stream_one", Method::Get, move |req| {
                const BOUNDARY: &str = "frame";
                let headers = [
                    ("Content-Type", "multipart/x-mixed-replace; boundary=frame"),
                    ("Cache-Control", "no-cache"),
...
<title>Cyberpunk Live Feed</title>
<style>
    ...
    body {{
        ...
        align-items: center;
        background: radial-gradient(circle at 30% 30%, #0f2027, #09141a 60%, #000000);
    ...
    #camera {{
        ...
        border-radius: 10px;
        box-shadow: 0 0 20px #00ffff88, 0 0 40px #00ffff44;
        animation: glowPulse 2s ease-in-out infinite alternate;
    }}

    @keyframes glowPulse {{
        0% {{ box-shadow: 0 0 10px #00ffff66, 0 0 30px #00ffff33; }}
        100% {{ box-shadow: 0 0 30px #00ffffcc, 0 0 80px #00ffff55; }}
    }}
    ...
    .hud-line {{
        ...
        animation: hudLine 5s linear infinite;
        opacity: 0.5;
    }}
...
<body>
    <h1>ESP32 CYBERCAM FEED</h1>
    <div id="clock"></div>
    <img id="camera" src="/stream" alt="Live camera feed">
    <footer>NeuralNet Interface v2.3 | Powered by ESP32-CAM</footer>
